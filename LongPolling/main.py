...
async def status(jobid):
    ...
    while not await checkJobComplete(jobid):
        pass
    return JSONResponse({"JobStatus": "Complete " + str(jobs[jobid])})


async def checkJobComplete(jobid):
    if jobs[jobid] < 100:
        time.sleep(1)
        await checkJobComplete(jobid)
        return False
    else:
        return True
